...
X1_test = random('Rayleigh',SIGMA,NN,1);
Y = [1:0.01:8];
Y_size = size(Y);
for i=1:Y_size(2)
l1(i) = 0;
...
%----2.3--—
sum_ocenka_sigma=sum(sigma)/NN;
smesh_ocenka=sum_ocenka_sigma-SIGMA;
disp=sum((sigma-sum_ocenka_sigma).^2)/NN;
rass=sum((sigma-SIGMA).^2)/NN;
%----3---—
figure(3);
hold all;
N1=10000;
...
for i=1:N1
sigmaa= mean(X)/(sqrt(pi/2));
end
sumsigma=sum(sigmaa);
title('Гистограмма и плотность вероятности')
f=pdf('Rayleigh',xx,SIGMA);
f1=pdf('Rayleigh',xx,sumsigma);
hold on
plot(xx,f,'red');
plot(xx,f1,'green')
%----4---—
figure(5);
hold all;
F_teor = cdf ('Rayleigh',xx,SIGMA);
F_teor_1=cdf('Rayleigh',xx,sumsigma);
